...
function StaffManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {staff && staff.slice(0, 7).map((member: any, index: number) => (
          <div key={`staff-dashboard-${member.id}-${index}`} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <table className="min-w-full divide-y divide-gray-200 rounded-lg overflow-hidden shadow-sm">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredStaff.map((member: any, index: number) => (
                  <motion.tr
                    key={`staff-list-${member.id}-${index}-${member.email}`}
